...
export async function resizeImage(
  ...
  return retryWithBackoff(async () => {
    ...
    const targetHeight =
      options.height || config.workflow?.resize?.height || 1080;
    const maintainRatio =
      options.maintainRatio !== undefined ? options.maintainRatio : true;
...
export async function compressImage(inputPath, outputPath, options = {}) {
  return retryWithBackoff(async () => {
    ...
    const compressionLevel =
      ...
      config.iloveimg?.defaults?.compressionLevel ||
      "recommended";
...
export async function watermarkImage(inputPath, outputPath, options = {}) {
  ...
  return retryWithBackoff(async () => {
    if (options.mode === "image") {
      ...
      logger.warn(
        "Text watermark not fully supported with Sharp, copying file",
      );
...
export async function processImages(images, options = {}) {
  ...
  if (parallel) {
    logger.info(`Processing... (parallel, concurrency: ${concurrency})...`);
    ...
    try {
      ...
      logger.warn(
        "p-limit package not available, falling back to sequential processing",
      );
    ...
    const promises = images.map((image, i) =>
      limit(async () => {
        logger.info(
          `  Processing image ${i + 1}/${images.length}: ${image.id}`,
        );
        try {
          ...
          logger.error(
            `  Failed to process image ${image.id}: ${error.message}`,
          );
          return {
            ...image,
            success: false,
